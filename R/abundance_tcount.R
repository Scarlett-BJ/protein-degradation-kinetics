...
ranked_tissues <- arrange(count(df_protdb_human, tissue), desc(n))$n
...
plot_binned_boxplots(filter(df_protdb_human, tissue == "testis"))
plot_binned_boxplots(filter(df_protdb_human, tissue %in% ranked_tissues[1:4])) +
  facet_grid(.~ tissue)
...
plot_linear(filter(df_protdb_human, tissue == ))
plot_linear(df_protdb_human)
plot_linear(df_protdb_human) + facet_wrap(~ tissue, nrow = 4)
